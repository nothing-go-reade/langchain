...
- `libs/core/langchain_core/runnables/config.py`
  - `RunnableConfig`：`TypedDict(total=False)`，字段覆盖 tags、metadata、callbacks、run_name、max_concurrency、recursion_limit、configurable、run_id。
  - `ensure_config`：补齐默认值，继承 `var_child_runnable_config`，把未知 key 放入 `configurable`，并把 `model`、`checkpoint_ns` 等部分 configurable 信息补入 metadata。
  - `_get_langsmith_inheritable_metadata_from_config`：只把简单值转成 LangSmith metadata，并排除 `api_key`。
  - `patch_config`：更新 callbacks、recursion_limit、max_concurrency、run_name、configurable；替换 callbacks 时清理 run_name/run_id。
  - `set_config_context` / `_set_config_context`：通过 `ContextVar` 传递 child config，并同步 LangSmith tracing parent。
  - `get_callback_manager_for_config` / `get_async_callback_manager_for_config`：从 config 构造可继承 callback manager。
  - `ContextThreadPoolExecutor` / `run_in_executor`：通过 `copy_context` 保持并发、线程池场景下的上下文传播。
  - `get_config_list`：batch 场景下对 config 列表长度和 run_id 做边界处理。
- `libs/core/langchain_core/callbacks/manager.py`
  - `CallbackManager.on_chain_start`：创建 run_id，分发 on_chain_start，并返回 `CallbackManagerForChainRun`。
  - `CallbackManagerForChainRun.on_chain_end` / `on_chain_error`：收束一次 chain run。
  - `ParentRunManager.get_child`：创建 parent_run_id 指向当前 run 的 child callback manager，继承 handlers/tags/metadata，并附加局部 tag。
- `libs/core/langchain_core/runnables/base.py`
  - `RunnableSequence.invoke`：root run -> `seq:step:n` child callbacks -> `set_config_context` -> step invoke。
  - `RunnableParallel.invoke`：为每个 key 创建 `map:key:<key>` child callback，并发执行分支。
  - `_transform_stream_with_config`：启动 run、tap output iterator、产出 stream callback、累积最终 input/output。
  - `astream_events`：v2 事件包含 event/name/run_id/parent_ids/tags/metadata/data。
- `libs/core/langchain_core/tracers/event_stream.py`
  - `_AstreamEventsCallbackHandler`：维护 `run_map` 与 `parent_map`，通过 `_get_parent_ids` 还原父链路；tap stream output 产出 `on_*_stream` 事件。
