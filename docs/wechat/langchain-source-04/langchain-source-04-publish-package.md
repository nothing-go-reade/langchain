# LangChain源码解析04：Message不只是字符串 发布包
...
第四篇聚焦 LangChain Message 体系：解释 `BaseMessage` 核心字段、`HumanMessage` / `SystemMessage` / `AIMessage` / `ToolMessage` 的职责，`content_blocks` 如何统一多模态内容，`AIMessage.tool_calls` 与 `ToolMessage.tool_call_id` 如何构成工具调用闭环，以及 provider translator 如何桥接不同模型 API 格式。

## 文件清单

- HTML 粘贴版：`docs/wechat/langchain-source-04/langchain-source-04-wechat.html`
- WeChat Markdown：`docs/wechat/langchain-source-04/langchain-source-04-wechat.md`
- 源稿 Markdown：`docs/wechat/langchain-source-04/langchain-source-04.md`
- Message 数据模型图：`docs/wechat/langchain-source-04/assets/message-data-model.png`
- Tool call 消息闭环图：`docs/wechat/langchain-source-04/assets/message-tool-call-loop.png`
