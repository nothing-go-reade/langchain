# LangChain源码解析06：Prompt和Parser守住两端 发布包
...
第六篇聚焦 LangChain 的输入输出契约：解释 `BasePromptTemplate` 如何通过 `input_variables`、`optional_variables`、`partial_variables` 和 `PromptValue` 约束模型输入，`ChatPromptTemplate` 与 `MessagesPlaceholder` 如何把历史消息和多模态模板统一成标准 Message；同时解释 `BaseOutputParser`、`StrOutputParser`、`JsonOutputParser`、`PydanticOutputParser` 和 tools parser 如何把模型输出收束成字符串、JSON、Pydantic 对象或 tool call 结构。

## 文件清单

- HTML 粘贴版：`docs/wechat/langchain-source-06/langchain-source-06-wechat.html`
- WeChat Markdown：`docs/wechat/langchain-source-06/langchain-source-06-wechat.md`
- 源稿 Markdown：`docs/wechat/langchain-source-06/langchain-source-06.md`
- Prompt/Parser 契约图：`docs/wechat/langchain-source-06/assets/prompt-parser-contract.png`
- Prompt/Parser 细节图：`docs/wechat/langchain-source-06/assets/prompt-parser-details.png`
