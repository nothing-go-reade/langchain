...
- `libs/core/langchain_core/prompts/base.py`
  - `BasePromptTemplate` 继承 `RunnableSerializable[dict[str, Any], PromptValue]`。
  - 维护 `input_variables`、`optional_variables`、`input_types`、`output_parser`、`partial_variables`、`metadata`、`tags`。
  - `_validate_input` 支持单变量快捷输入，缺变量时报错并提示双花括号转义。
  - `invoke` / `ainvoke` 用 `run_type="prompt"` 进入 Runnable tracing。
  - `get_input_schema` 根据 required / optional variables 生成 Pydantic schema。
- `libs/core/langchain_core/prompts/prompt.py`
  - `PromptTemplate` 支持 `f-string`、`mustache`、`jinja2`。
  - `from_template` 自动推断变量，并排除 `partial_variables`。
  - `validate_template=True` 时校验模板变量一致性。
- `libs/core/langchain_core/prompts/string.py`
  - `validate_f_string_template` 禁止属性访问、索引访问、纯数字变量和嵌套 replacement。
  - `mustache_schema` 可为嵌套变量生成 Pydantic 输入模型。
  - `jinja2_formatter` 使用 sandbox，但明确提示不要接收不可信模板。
- `libs/core/langchain_core/prompts/chat.py`
  - `ChatPromptTemplate` 支持多种 message-like 表达，自动推断输入变量。
  - `MessagesPlaceholder` 可插入历史消息，支持 optional 和 `n_messages`。
  - 多模态消息模板会产出标准 message content list。
- `libs/core/langchain_core/prompt_values.py`
  - `StringPromptValue` 和 `ChatPromptValue` 都支持 `to_string()` / `to_messages()`。
- `libs/core/langchain_core/prompts/structured.py`
  - `StructuredPrompt` 在 pipe 到模型时会调用 `with_structured_output(schema)`。
- `libs/core/langchain_core/output_parsers/base.py`
  - `BaseOutputParser` 继承 Runnable，输入可以是 `str` 或 `BaseMessage`。
  - `invoke` 会把消息包装成 `ChatGeneration`，把字符串包装成 `Generation`。
  - `parse_result` 默认只解析第一个 candidate。
- `libs/core/langchain_core/output_parsers/transform.py`
  - `BaseTransformOutputParser` 支持流式 transform。
  - `BaseCumulativeTransformOutputParser` 会累积 chunk，支持 partial 和 diff。
- `libs/core/langchain_core/output_parsers/json.py`
  - `JsonOutputParser` 支持 Markdown JSON 抽取、partial JSON、JSONPatch diff。
- `libs/core/langchain_core/output_parsers/pydantic.py`
  - `PydanticOutputParser` 先 JSON parse，再 Pydantic validate，兼容 v1/v2。
- `libs/core/langchain_core/output_parsers/openai_tools.py`
  - `JsonOutputToolsParser` / `PydanticToolsParser` 解析 tool call 输出并校验工具参数对象。
- `libs/core/tests/unit_tests/prompts/*`
  - 覆盖变量推断、partial、MessagesPlaceholder、mustache 安全边界、多模态 dict 模板。
- `libs/core/tests/unit_tests/output_parsers/*`
  - 覆盖 Pydantic parser chaining、JSON markdown 解析、streaming partial/diff、tools parser 错误。

## 维护提示

- 公众号公开版不要暴露本节内容。
- 第 1-6 篇公开 URL 已同步至系列链接。
